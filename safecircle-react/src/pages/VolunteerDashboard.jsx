...
export default function VolunteerDashboard() {
  ...
  const drawRoute = async (victimLat, victimLon) => {
    ...
    try {
      ...
      if (routeData.route) {
        ...
        routeLine.current = L.polyline(routeData.route, { color: "#ef4444", weight: 4 }).addTo(
          mapInstance.current
        );
  ...
  useEffect(() => {
    ...
    socket.on("victim-location-update", (data) => {
      ...
      if (victimSocketId.current) {
        dispatch(
          setActiveMission({ victimSocketId: victimSocketId.current, lat: data.lat, lon: data.lon })
        );
      }
  ...
  const acceptAlert = async (alert) => {
    ...
    dispatch(
      setActiveMission({ victimSocketId: alert.victimSocketId, lat: alert.lat, lon: alert.lon })
    );
    ...
    <div className="home-page vd-page">
        ...
        <div className="vd-header-top">
          <div className="vd-header-left">
            ...
            <div>
              ...
              <p className="vd-subtitle">
                {isOnline
                  ? `Responding as ${volunteerName} · receiving emergency alerts`
          ...
          <div
            className={`vd-status-pill ${
              isOnline ? "vd-status-pill--online" : "vd-status-pill--offline"
            }`}
          >
          ...
          <div className="vd-name-row">
            <div className="vd-input-group">
              ...
              <div className="vd-input-wrapper">
                <span className="vd-input-icon">👤</span>
                <input
                  className="vd-input"
                  value={volunteerName}
            ...
            <div className="vd-toggle-wrapper">
              {isOnline ? (
                <button className="vd-power-btn vd-power-btn--off" onClick={goOffline}>
                  <span className="vd-power-icon">⏻</span>
              ...
              <div className="admin-ai-emotion-badge">
                ...
                <div>
                  ...
                  <div className="admin-ai-intensity-text">
                    ...
                    {liveAI.keyword_triggered && (
                      <span className="admin-ai-keyword-flag">⚠️ keyword detected</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="dashboard-grid">
          ...
          <div className="admin-section-body scrollable-feed">
            {liveAlerts.length === 0 ? (
              <div className="admin-empty-state">
                <span className="admin-empty-icon">{isOnline ? "📡" : "🔌"}</span>
                <p>
                  {isOnline
                    ? "No active alerts nearby. Stay on standby."
                  ...
                  <div key={al.id} className="vd-alert-card">
                        ...
                        <div>
                          ...
                          <div className="vd-alert-meta">
                            ...
                            {al.lat && (
                              <span>
                                {" "}
                                · {Number(al.lat).toFixed(3)}°N, {Number(al.lon).toFixed(3)}°E
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      {sevLevel && (
                        <div
                          ...
                          style={{
                            ...
                            animation:
                              sevLevel === "critical" ? "pulse-red 1.5s infinite" : "none",
                          }}
                        >
                          {sevLevel === "critical" && "🔴 "}
                          {sevLevel === "high" && "🟠 "}
                    ...
                    {sev && sev.emotion && (
                      <div className="vd-alert-ai-strip">
                        ...
                        <span className="vd-ai-strip-pct">
                          {(sev.intensity * 100).toFixed(0)}%
                        </span>
        ...
        <div className="card admin-section-card">
          <div className="admin-section-header">
            <h2>📋 Activity Log</h2>
            <span className="admin-section-count admin-section-count--green">
              {activityLog.length}
            </span>
          </div>
          <div className="admin-section-body scrollable-feed">
        ...
        <div className="admin-section-header">
          ...
          <div className="admin-map-legend">
            <span className="admin-legend-item">
              <span className="admin-legend-dot admin-legend-dot--green"></span>You
            </span>
            <span className="admin-legend-item">
              <span className="admin-legend-dot admin-legend-dot--red"></span>Victim
            </span>
          </div>
        </div>
